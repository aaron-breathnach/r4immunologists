...
Wikipedia describes R as "a programming language for statistical computing and graphics".
...
Use the `cube_root()` function to find the cube root of 1728.
...
Use the `nth_root()` function to get the 4<sup>th</sup> root of 1296:
...
Write a loop to find the cube root of the following values: 62, 92, and 37.
...
### Pacakges for today's workshop

For today's workshop, we'll need to install a few packages:

```{r eval=FALSE}
packages <- c("tidyverse", "scales", "rstatix", "ggpubr", "ComplexHeatmap")
...
The tibble package provides us with the ability to use tibbles, which are data frames with ["more user-friendly printing, subsetting, and factor handling"](https://www.oreilly.com/library/view/efficient-data-processing/9781491980736/ch01.html). Tibbles makes it easier to work with large data.

Run the code below to see the differences between a data frame and a tibble:
...
The dplyr package can be used to transform our data. To demonstrate some of dplyr's functionalities, I'm going to use the starwars dataset which comes packaged with dplyr. We'll get back to the immunology shortly, but I couldn't pass up this opportunity.
...
In this example, we will import data from the file `murphy.tsv` using `read_tsv()` function from readr. Note that the data used to generate all of the figures for the [Murphy *et al.* (2022)](https://www.jci.org/articles/view/162581) paper has been collated in this table.
...
- <span style="color: #f74ed6;">#f74ed6</span> for F

- <span style="color: #1199b2;">#1199b2</span> for M
...
To facet this plot, we use `facet_wrap()`, as follows:
...
Here, I'll show you how to make a heatmap using the `pheatmap()` function from the ComplexHeatmap package so that you can impress your friends.

In this example, I'll use the `metabolomics.tsv` dataset, which includes the concentrations of 25 metabolites in 20 mice: 10 mice were fed a standard-fat diet (SFD), and 10 mice were fed a high-fat diet (HFD).
...
We can do a square root transformation on the the matrix as follows:
...
ComplexHeatmap::pheatmap(log_mat,
         scale = "row",
         name = "z-score")
```
...
Here, we will use Tukey's test with ANOVA of the data used to generate Figure 3C from the Murphy _et al_. (2022) study, which will tell us if CD14<sup>+</sup> CD16<sup>+</sup> cell counts change significantly over time relative to Day -1.
...
rho <- round(corr[[1, "cor"]], 3)

label <- paste0("\U03C1 = ", rho)
...
p <- ggplot(eg, aes(x = relab, y = mfi)) +
  ...
  geom_point(pch = 21, colour = "black", fill = "forestgreen") +
  geom_text(x = x_pos,
            y = y_pos,
            label = label,
            hjust = 0,
            size = 6.25) +
  labs(x = "*Bifidobacterium pseudolongum* (%)",
       y = paste0("Neutrophils (\U03BC", "l<sup>-1</sup>)")
  ) +
...
As you can see, the first column contains merged cells. When we import this table, these cells will be unmerged as shown below:
...
Now, it's straightforward to work with the table. For example, we can make a box plot with ggplot:

```{r}
tnf_long <- tnf %>%
  pivot_longer(cols = !c(group, ct), names_to = "sample_id", values_to = "tnf")

ggplot(tnf_long, aes(x = group, y = tnf, fill = as.factor(ct))) +
  geom_boxplot() +
  labs(x = "Treatment", y = "TNF", fill = "Concentration")
```
